...
const requestSchema = z.object({
  ...
  masterProfile: z.object({
    ...
    fonts: z.object({ major: z.string().max(80).optional(), minor: z.string().max(80).optional() }),
    assetCount: z.number().int().min(0).max(100),
    decorations: z.array(z.discriminatedUnion("kind", [
      z.object({ kind: z.enum(["rect", "ellipse"]), x: z.number(), y: z.number(), width: z.number().positive(), height: z.number().positive(), color: z.string().regex(/^#[0-9A-Fa-f]{6}$/), opacity: z.number().min(0).max(1).optional() }),
      z.object({ kind: z.literal("line"), x: z.number(), y: z.number(), width: z.number(), height: z.number(), color: z.string().regex(/^#[0-9A-Fa-f]{6}$/), opacity: z.number().min(0).max(1).optional(), lineWidth: z.number().positive().max(20).optional() }),
      z.object({ kind: z.literal("text"), value: z.string().max(500), x: z.number(), y: z.number(), width: z.number().positive(), height: z.number().positive(), fontSize: z.number().positive().max(200), color: z.string().regex(/^#[0-9A-Fa-f]{6}$/), fontFamily: z.string().max(80), weight: z.number().min(100).max(900), align: z.enum(["left", "center", "right"]).optional() }),
      z.object({ kind: z.literal("image"), x: z.number(), y: z.number(), width: z.number().positive(), height: z.number().positive(), assetId: z.string().regex(/^asset-\d+\.(png|jpg|gif)$/i), assetUrl: z.string().regex(/^\/api\/presentations\/templates\/assets\/[0-9a-f-]{36}\/asset-\d+\.(png|jpg|gif)$/i) }),
    ])).max(200),
  ...
  slides: z
    .array(
      z.object({
        ...
        layout: z.enum(["cover", "statement", "split", "list", "comparison", "timeline", "quote", "closing", "chart", "table"]).optional(),
        ...
        layoutAlternatives: z.array(z.enum(["cover", "statement", "split", "list", "comparison", "timeline", "quote", "closing", "chart", "table"])).max(4).optional(),
        chart: z.object({
          type: z.enum(["bar", "line", "pie"]),
          categories: z.array(z.string().max(80)).min(1).max(12),
          series: z.array(z.object({ name: z.string().max(80), values: z.array(z.number().finite()).min(1).max(12) })).min(1).max(6),
        }).optional(),
        table: z.object({
...
export async function POST(request: Request) {
  try {
    ...
    const orchestration = input.layoutMode === "auto" ? orchestratePresentation(input.slides) : null;
    ...
    arrangedSlides.forEach((slide, index) => {
      ...
      if (slide.templateId && resolvePresentationTemplate(layout, slide.templateId).id !== slide.templateId) {
        throw new Error(`Slide ${index + 1} template does not match its layout.`);
      }
      if (layout === "chart" && !slide.chart) throw new Error(`Slide ${index + 1} requires chart data.`);
      if (layout === "table" && !slide.table) throw new Error(`Slide ${index + 1} requires table data.`);
      if (slide.chart && slide.chart.series.some((series) => series.values.length !== slide.chart!.categories.length)) {
      ...
      if (slide.table && slide.table.rows.some((row) => row.length !== slide.table!.headers.length)) {
    ...
    if (input.masterProfile && input.masterProfile.decorations.some((decoration) => decoration.kind === "image" && !decoration.assetUrl.includes(`/${input.masterProfile!.id}/`))) {
